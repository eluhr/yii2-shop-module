...
?>
    ...
    <div class="form-group">
        <div class="btn-toolbar">
            ...
            <?= Html::a(Yii::t('shop', '{icon} New', ['icon' => FA::icon(FA::_PLUS)]), ['product-edit'], ['class' => 'btn btn-success']) ?>
        </div>
    </div>
...
echo GridView::widget([
    ...
    'columns' => [
        [
            ...
            'urlCreator' => function (string $action, Product $model) {
        ...
        [
            ...
            'value' => function (Product $model) {
                ...
                foreach ($model->variants as $variant) {
                    $html[] = Html::tag('p', ($variant->fewAvailable() ? FA::icon(FA::_EXCLAMATION, ['class' => 'text-warning']) . ' ' : '') . $variant->title . ' ' . FA::icon(FA::_CIRCLE, ['class' => 'text-' . ($variant->is_online ? 'success' : 'danger')]) . ' ' . Html::a(FA::icon(FA::_PENCIL), ['variant-edit', 'id' => $variant->id], ['class' => 'btn btn-xs btn-primary']));
                }
                $html[] = Html::a(Yii::t('shop', '{icon} Add Variant', ['icon' => FA::icon(FA::_PLUS)]), ['variant-edit', 'product_id' => $model->id], ['class' => 'btn btn-xs btn-success']);
                return implode(PHP_EOL, $html);
            },
            'format' => 'html'
        ],
